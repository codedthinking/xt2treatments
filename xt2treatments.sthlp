...
{text}{phang2}{cmd:xt2treatments} varname(numeric) [{it:if}], {bf:treatment}(varname numeric) {bf:control}(varname numeric), [{bf:pre}(#) {bf:post}(#) {bf:baseline}({it:string}) {bf:weighting}(string) {bf:cluster}(varname) {bf:graph}]{p_end}
